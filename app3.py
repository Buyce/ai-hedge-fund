...
st.markdown("Generate 18 deep-dive financial, strategic, and macro reports using Google's flagship Gemini 3.1 Pro model.")

# --- 2. THE SIDEBAR (ADMIN VIEW) ---
with st.sidebar:
    st.header("⚙️ Settings")
    st.info("API Key is managed by the administrator.")
...
# --- 3. THE USER INPUTS ---
user_email = st.text_input("📧 Enter your email to unlock the report:")
...
# Let the user choose their download format!
download_format = st.radio("Select Output Format:", ["PDF (.pdf)", "Word Document (.doc)"])
...
# --- INITIALIZE MEMORY ---
# This ensures the variables exist before the user clicks anything
...
if st.button("🚀 Generate Master Hedge Fund Report", use_container_width=True):
    ...
    if not user_email or "@" not in user_email:
        st.error("Please enter a valid email address to unlock the research.")
        st.stop()
    if not target_company or not target_ticker:
        st.error("Please provide at least a Company Name and Ticker Symbol.")
        st.stop()
        
    save_lead(user_email, target_ticker)

    client = genai.Client(api_key=st.secrets["GOOGLE_API_KEY"])

    def run_agent(agent_name, raw_instruction, additional_context=""):
        # Skipping Logic
        if agent_name in industry_agents and not target_industry.strip(): 
            return agent_name, "Skipped: No Industry provided."
        if agent_name in concept_agents and not target_concept.strip(): 
            return agent_name, "Skipped: No Concept provided."
        if agent_name in ceo_agents and not target_ceo.strip(): 
            return agent_name, "Skipped: No CEO name provided."

        instruction = raw_instruction.replace("[STOCK NAME]", target_company).replace("[TICKER]", target_ticker).replace("[Company_name]", target_company).replace("{Company_Name}", target_company).replace("[INSERT INDUSTRY]", target_industry).replace("[Industry Name]", target_industry).replace("{CONCEPT NAME}", target_concept).replace("{{CEO Name}}", target_ceo).replace("{{Company Name}}", target_company)

        try:
            if additional_context:
                # STAGE 2: Synthesis (Ticker Analyst & Macro)
                prompt = f"YOU ARE A SYNTHESIS AGENT. USE THE RESEARCH BELOW TO ANSWER:\n\n{instruction}\n\nRESEARCH DATA:\n{additional_context}"
                response = client.models.generate_content(
                    model='gemini-3.1-pro-preview', # Using a stable, high-context model for synthesis
                    contents=prompt,
                ...
                prompt = f"{instruction}\n\nTarget: {target_company} ({target_ticker})"
                ...
                while True:
                    ...
                    if interaction.status == "completed":
                        return agent_name, interaction.outputs[-1].text
                    elif interaction.status == "failed":
                        return agent_name, f"Deep Research Failed: {interaction.error}"
                    time.sleep(15) 
    ...
    # --- STAGE 1: EXECUTION ---
    st.info("Stage 1: Running Deep Research Agents...")
    base_agents = {name: prompt for name, prompt in gem_prompts.items() if name not in dependent_agents}
    
    progress_bar = st.progress(0)
    completed_tasks = 0

    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_agent = {executor.submit(run_agent, name, prompt): name for name, prompt in base_agents.items()}
        for future in concurrent.futures.as_completed(future_to_agent):
            name, result_text = future.result()
            completed_tasks += 1
            progress_bar.progress(int((completed_tasks / len(gem_prompts)) * 100))
            if result_text:
                st.session_state.final_reports[name] = result_text

    # --- PREPARE DATA FOR STAGE 2 ---
    # We explicitly build the context string AFTER Stage 1 is 100% finished
    full_context = ""
    for name in base_agents.keys():
        if name in st.session_state.final_reports:
            report_body = st.session_state.final_reports[name]
            if "Skipped:" not in report_body and "Error:" not in report_body:
                full_context += f"\n\n=== SOURCE REPORT: {name} ===\n{report_body}\n"

    # --- STAGE 2: EXECUTION ---
    if full_context.strip():
        st.info("Stage 2: Finalizing Synthesis (Ticker & Macro)...")
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            # We pass the 'full_context' we just built into these agents
            future_to_agent = {executor.submit(run_agent, name, gem_prompts[name], full_context): name for name in dependent_agents}
            for future in concurrent.futures.as_completed(future_to_agent):
                name, result_text = future.result()
                if result_text:
                    st.session_state.final_reports[name] = result_text
    else:
        st.warning("Stage 2 skipped: No valid research data was found from Stage 1.")

    st.session_state.analysis_complete = True
    st.rerun()


# --- 6. DISPLAY & DOWNLOAD (OUTSIDE THE BUTTON BLOCK!) ---
# By putting this outside the button block, it survives the page refresh!
if st.session_state.analysis_complete:
    st.success("Analysis Complete!")
    
    st.header("📑 Your Reports")
    for name, text in st.session_state.final_reports.items():
        with st.expander(f"View Report: {name}"):
            st.markdown(text)

    # --- 7. CREATE A .ZIP DOWNLOAD BUTTON ---
    import markdown
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for name, text in st.session_state.final_reports.items():
            ...
            # Skip the brittle PDF conversion for now. Stick to the bulletproof MS Word hack.
            ...
            zip_file.writestr(f"{target_ticker}_{safe_name}.doc", doc_content.encode('utf-8'))
    
    st.download_button(
        label=f"📥 Download All 18 Reports as .DOC",
        data=zip_buffer.getvalue(),
        file_name=f"{target_ticker}_AI_HedgeFund_Reports.zip",
